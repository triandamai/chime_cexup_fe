<template>
  <div>
    ...
    >
      ...
      <span class="absolute top-0 right-0" @click="error = !error">
        <svg
...
import { A_CREATE } from "../store/session.module";
...
export default {
  ...
  computed: {
    ...mapState({
      error: state => state.errorAuth,
      message: state => state.messageAuth
    })
  },
  methods: {
    joinMeeting(e) {
      ...
      if (!this.username || !this.password) {
        this.$store.commit(SET_MESSAGE_AUTH, "Mohon isi semua field");
        this.$store.commit(SET_ERROR_AUTH, true);
      } else {
        this.$store.dispatch(A_CREATE, {
          username: this.username,
          password: this.password,
          userid: Date.now(),
          description: "host meeting doctor"
        });
      }
    }
  }
